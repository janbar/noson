...
bool TcpSocket::SendData(const char* msg, size_t size)
{
  if (IsValid())
  {
    ...
    {
      pthread_sigmask(SIG_SETMASK, &sig_restore, nullptr);
      return false;
    }
...
std::string TcpSocket::GetHostAddrInfo()
{
  ...
  if (getsockname(m_socket, (sockaddr*)&addr, &addr_len) == 0)
    getnameinfo((sockaddr*)&addr, addr_len, host, sizeof(host), nullptr, 0, NI_NUMERICHOST);
  else
    m_errno = LASTERROR;
...
std::string TcpServerSocket::GetRemoteAddrInfo()
{
  ...
  getnameinfo(m_addr->sa(), m_addr->sa_len, rhost, sizeof(rhost), nullptr, 0, NI_NUMERICHOST);
  return rhost;
}
...
std::string UdpSocket::GetRemoteAddrInfo() const
{
  char host[INET6_ADDRSTRLEN];
  memset(host, 0, INET6_ADDRSTRLEN);
  getnameinfo(m_from->sa(), m_from->sa_len, host, sizeof(host), nullptr, 0, NI_NUMERICHOST);
  return host;
}
...
bool UdpServerSocket::Bind(unsigned port, bool reuse /*= false*/)
{
  if (!IsValid())
    return false;
...
#ifdef __WINDOWS__
  // disable the option SO_EXCLUSIVEADDRUSE to allow reuse.
  int opt = (reuse ? 0 : 1);
  if (setsockopt(m_socket, SOL_SOCKET, SO_EXCLUSIVEADDRUSE, (char*)&opt, sizeof(opt)))
  {
    m_errno = LASTERROR;
...
bool UdpServerSocket::Bind(const char* addr, unsigned port, bool reuse /*= false*/)
{
  if (!IsValid())
    return false;

  unsigned char _addr[sizeof(struct in6_addr)];
  if (inet_pton(m_addr->sa_family(), addr, &_addr) == 0)
  {
    m_errno = LASTERROR;
    DBG(DBG_ERROR, "%s: invalid address (%s)\n", __FUNCTION__, addr);
    return false;
